...
\begin{eqnarray*}
\ensuremath{\Conid{Block}\mathbin{::}(\Conid{MagicNo},(\Conid{Time},\Conid{Transactions}))}
\end{eqnarray*}
...
Assim, após termos a \ensuremath{[\mskip1.5mu \Conid{Transaction}\mskip1.5mu]} (resultado da função \ensuremath{\Varid{allTransactions}})
aplicamos-lhe um \ensuremath{\Varid{cataList}}.
...
Para cada \ensuremath{\Conid{Transaction}} verificada são adicionados dois novos pares à lista: o primeiro
...
como enviados) e respetivas entidades aplicamos-lhe uma função chamada \ensuremath{\Varid{groupL}}.

A função \ensuremath{\Varid{groupL}} ordena a \ensuremath{[\mskip1.5mu (\Conid{Entity},\Conid{Value})\mskip1.5mu]}, agrupa devidamente os
seus elementos e por fim devolve \ensuremath{[\mskip1.5mu (\Conid{Entity},\Conid{Value})\mskip1.5mu]} mas sem repetições
...
Consequentemente, temos a função \ensuremath{\Varid{ledger}} definida:
...
\\
\>[5]{}\hsindent{6}{}\<[11]%
\>[11]{}\Varid{groupL}\;\Varid{t}\mathrel{=}(\Varid{sums}\comp \map \;(\Varid{mapFst}\;\Varid{head}\comp \Varid{unzip})\comp \Varid{groupBy}\;(\lambda \Varid{x}\;\Varid{y}\to \p1\;\Varid{x}\equiv \p1\;\Varid{y})\comp \Varid{sort})\;\Varid{t}{}\<[E]%
\\
\>[5]{}\hsindent{6}{}\<[11]%
\>[11]{}\Varid{mapFst}\;\Varid{f}\;(\Varid{a},\Varid{b})\mathrel{=}(\Varid{f}\;\Varid{a},\Varid{b}){}\<[E]%
\\
\>[5]{}\hsindent{6}{}\<[11]%
...
através de um \ensuremath{\Varid{cataBlockchain}} com um gene:
\begin{eqnarray*}
\start
...
obtivesse uma lista com todos os números mágicos utilizados na \ensuremath{\Conid{Blockchain}}.
...
Por fim, aplicamos \ensuremath{\Varid{all}\;((\equiv )\;\mathrm{1}\comp \length )}. O que acontece é que calculamos o comprimento
de todas as listas dentro da lista maior e verificamos se os seus comprimentos são iguais a 1.
Assim, caso haja uma lista com vários números mágicos a função retornará \ensuremath{\Conid{False}}, tal como
é esperado.

A função \ensuremath{\Varid{isValidMagicNr}} pode ser então definida como:
...
Assim, percebemos de imediato que a função \ensuremath{\Varid{baseQTree}} terá que ser definida como
...
da \ensuremath{\Conid{Cell}} (lado esquerdo do \ensuremath{\mathbin{+}} inferior) (através da função \ensuremath{\Varid{f}}) e de juntar
tudo numa só \ensuremath{\Conid{QTree}}.
...
nível desejado e em caso afirmativo eliminasse toda a \ensuremath{\Conid{QTree}} a partir desse nível.


Assim, passamos para o desenvolvimento do problema:

Temos por exemplo \ensuremath{\Conid{QTree}} da Figura~\ref{fig:qtt},
que é representada pela \ensuremath{\Conid{QTree}\;\Varid{qt}}, que já estava presente no enunciado:
\begin{eqnarray*}
\start
...
dois níveis, o esquema da \ensuremath{\Conid{QTree}} de retorno deverá ser
...
os ramos da \ensuremath{\Conid{QTree}} a abolir. Percebemos através da Figura~\ref{fig:qtt}
...
\ensuremath{\Conid{Qtree}} a eliminar deveremos transformá-la numa única \ensuremath{\Conid{Cell}}.
...
deverá ser cortada (a sua raiz é aplicada à função \ensuremath{\Varid{cortar}}) ficando somente a \ensuremath{\Conid{Cell}}
comprimida.

É ainda de mencionar que, tendo em conta o tipo de \ensuremath{\Varid{anaQTree}}, a nossa \ensuremath{\Varid{compress}}
deverá retornar sempre uma \ensuremath{\Conid{QTree}} ``aberta'' e, assim, aplicamos
a função \ensuremath{\Varid{outQTree}} tal como se pode ver na definição de \ensuremath{\Varid{compressQTree}}.
...
Esta função chama-se \ensuremath{\Varid{qt2bm}} e converte uma \ensuremath{\Conid{QTree}\;\Varid{a}} em \ensuremath{\Conid{Matrix}\;\Varid{a}}.
...
Assim, olhamos para o gene do \ensuremath{\Varid{cataQTree}} \ensuremath{\alt{\Varid{f}}{\Varid{g}}} e pensamos no que teremos
...
Neste caso, basta aplicar a função \ensuremath{matrix} onde o conteúdo de todos os
...
O quarto problema aborda \ensuremath{\Conid{Fractais}}. Fractais são formas geométricas que
...
Assim, analisando o tipo de uma \ensuremath{\Conid{FTree}} sabemos que uma:
\begin{eqnarray*}
\ensuremath{\Conid{FTree}\;\Varid{a}\;\Varid{b}}
\end{eqnarray*}
poderá ser
\begin{eqnarray*}
\ensuremath{\Conid{Unit}\;\Varid{b}}
\end{eqnarray*}
ou
\begin{eqnarray*}
\ensuremath{\Conid{Comp}\;\Varid{a}\;(\Conid{FTree}\;\Varid{a}\;\Varid{b})\;(\Conid{FTree}\;\Varid{a}\;\Varid{b})}
\end{eqnarray*}

Deste modo, as funções \ensuremath{\Varid{inFTree}} e \ensuremath{\Varid{outFTree}} são as seguintes:
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{5}{@{}>{\hspre}l<{\hspost}@{}}%
...
Diagrama de \ensuremath{\Varid{inFTree}}:
\begin{eqnarray*}
\xymatrix@C=2cm{
...
Diagrama de \ensuremath{\Varid{outFTree}}:
\begin{eqnarray*}
\xymatrix@C=2cm{
     \ensuremath{\Conid{FTree}\;\Varid{a1}\;\Varid{a2}}
           \ar[r]_-{\ensuremath{\Varid{outFTree}}}
&
     \ensuremath{\Varid{a2}+(\Varid{a1},(\Conid{FTree}\;\Varid{a1}\;\Varid{a2},\Conid{FTree}\;\Varid{a1}\;\Varid{a2}))}
}
\end{eqnarray*}
...
respeito a \ensuremath{\Conid{QTree}}, o pensamento para as definir foi muito semelhante.
...
Tal como nas funções \ensuremath{\Varid{inFTree}} e \ensuremath{\Varid{outFTree}}, estas quatro funções também
...
Tem o objetivo de alterar os argumentos da \ensuremath{\Conid{FTree}} aplicando-lhe funções
...
Temos ainda um \ensuremath{\Conid{Bifuntor}}:
...
O gene \ensuremath{\Varid{g}\mathrel{=}\Varid{inFTree}\comp \Varid{baseFTree}\;\Varid{f}\;\Varid{h}\;\Varid{id}} do \ensuremath{\Varid{cataFTree}} pode ser demonstrado
através do seguinte diagrama, onde assumimos que \ensuremath{\Varid{f}\mathbin{:}\Varid{a1}\to \Varid{b1}} e \ensuremath{\Varid{h}\mathbin{:}\Varid{a2}\to \Varid{b2}}.
\begin{eqnarray*}
\xymatrix@C=20cm{
    \ensuremath{\Varid{a2}+(\Varid{a1},(\Varid{a3},\Varid{a3}))}
       \ar[d]_-{\ensuremath{\Varid{baseFTree}\;\Varid{f}\;\Varid{h}\;\Varid{id}}}
\\
    \ensuremath{\Varid{b2}+(\Varid{b1},(\Varid{a3},\Varid{a3}))}
       \ar[d]_-{\ensuremath{\Varid{inFTree}}}
\\
    \ensuremath{\Conid{FTree}\;\Varid{b1}\;\Varid{b2}}
}
\end{eqnarray*}

Construindo o \ensuremath{\Varid{cataFTree}} aplicado ao gene referido anteriormente temos:
\begin{eqnarray*}
\xymatrix@C=3cm{
    \ensuremath{\Conid{FTree}\;\Varid{a1}\;\Varid{a2}}
           \ar[d]_-{\ensuremath{\Varid{cataFTree}\;\Varid{g}}}
&
    \ensuremath{\Varid{a2}+(\Varid{a1},(\Conid{FTree}\;\Varid{a1}\;\Varid{a2},\Conid{FTree}\;\Varid{a1}\;\Varid{a2}))}
           \ar[d]^{\ensuremath{\Varid{recFTree}\;(\Varid{cataFTree}\;\Varid{g})}}
           \ar[l]_-{\ensuremath{\Varid{inFTree}}}
\\
     \ensuremath{\Conid{FTree}\;\Varid{b1}\;\Varid{a2}}
&
     \ensuremath{\Varid{a2}+(\Varid{b1},(\Conid{FTree}\;\Varid{a1}\;\Varid{a2},\Conid{FTree}\;\Varid{a1}\;\Varid{a2}))}
           \ar[l]^-{\ensuremath{\Varid{g}}}
}
\end{eqnarray*}

Mais uma vez, as justificações do Problema 2 relaticamente à função \ensuremath{\mathsf{fmap}} fazem
um ``match'' com a função \ensuremath{\Varid{bimap}}, pelo que uma explicação mais detalhada
...
\begin{enumerate}
\item Função \ensuremath{\Varid{generatePTree}}

Esta função tem o objetivo de gerar uma árvore de Pitágoras para uma
dada ordem.

Tal como indica o enunciado, definimos esta função como uma \ensuremath{\Varid{anaFTree}}:
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{5}{@{}>{\hspre}l<{\hspost}@{}}%
...
O gene do \ensuremath{\Varid{anaFTree}}:
\begin{eqnarray*}
\start
\ensuremath{\Varid{g}\mathrel{=}((\underline{\mathrm{0}}\mathbin{+}\conj{\Varid{m}}{\conj{\Varid{id}}{\Varid{id}}})\comp \Varid{outNat}}
\more
\ensuremath{\Varid{m}\;\Varid{x}\mathrel{=}\mathrm{50}\mathbin{*}(\Varid{sqrt}\;(\mathrm{2})\mathbin{/}\mathrm{2})\mathbin{\uparrow}\Varid{abs}\;(\Varid{x}\mathbin{-}\Varid{a})}
\end{eqnarray*}
...
\xymatrix@C=20cm{
    \ensuremath{\Conid{Int}}
           \ar[d]_-{\ensuremath{\Varid{outNat}}}
\\
    \ensuremath{\mathrm{1}\mathbin{+}\Conid{Int}}
            \ar[d]_-{\ensuremath{(\underline{\mathrm{0}}\mathbin{+}\conj{\Varid{m}}{\conj{\Varid{id}}{\Varid{id}}})}}
\\
    \ensuremath{\underline{\mathrm{0}}\mathbin{+}(\Conid{Square},(\Conid{Int},\Conid{Int}))}
}
\end{eqnarray*}
...
\xymatrix@C=4cm{
    \ensuremath{\Conid{Int}}
        \ar[d]_-{\ensuremath{\Varid{anaList}\;\Varid{g}}}
        \ar[r]^-{\ensuremath{\Varid{g}\mathrel{=}\Varid{nil}\mathbin{+}\conj{\Varid{list}}{\Varid{id}}}}
&
    \ensuremath{\mathrm{1}+(\Conid{Picure}\;\Varid{x}\;\Conid{Int})}
           \ar[d]^{\ensuremath{\Varid{recList}\;(\Varid{anaList}\;\Varid{g})}}
\\
     \ensuremath{[\mskip1.5mu \Conid{Picture}\mskip1.5mu]}
&
     \ensuremath{\Varid{nil}+(\Conid{Picure},[\mskip1.5mu \Conid{Picture}\mskip1.5mu])}
           \ar[l]^-{\ensuremath{\Varid{inList}}}
}
\end{eqnarray}

Tal como sugere no enunciado, se corremos \ensuremath{\Varid{animatePTree}\;\mathrm{3}} os passos que vão aparecendo
